...
# Copy application files. Every module server.py imports must be listed: config
# (the env YAML) and results_store (where predictions are saved). A missing one
# only fails at container start, so keep this in sync.
COPY server.py config.py results_store.py .
